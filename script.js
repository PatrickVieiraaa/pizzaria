...
cs(".pizzaInfo--cancelMobileButton, .pizzaInfo--cancelButton").forEach((i) => {
...
c(".pizzaInfo--addButton").addEventListener("click", () => {
  ...
  //adicionanod informações no carrinho, vou adicionar no array criado no inicio do codigo, o 'Cart'
  updateCart();
  closeModal();
});
...
c(".cart--finalizar").addEventListener("click", finalizarCompra);
function finalizarCompra() {
  setTimeout(() => {
    c(".finaly").style.display = "block";
  }, 300);

  setTimeout(() => {
    c(".finaly").style.display = "none";
    cart = [];
    updateCart();
  }, 4000);
}
